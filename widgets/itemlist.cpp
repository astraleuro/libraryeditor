...
void ItemList::initData(QString fn, QJsonObject &data, QJsonObject &opt, JsonDataSections &sec)
{
    allSettings = opt;
...
QJsonArray ItemList::takeData()
{
    return jsonArray;
}

JsonDataSections ItemList::takeSection()
{
    return section;
}
...
QJsonObject ItemList::takeSettings()
{
    switch (section) {
    case ArtsSection:
...
void ItemList::changeItem(QJsonObject data, int row)
{
    QString key;
    QString newArg;
    if (row == -1) {
...
QString ItemList::jsonValueToText(QString key, QJsonValue value)
{
...
void ItemList::reorderItems(QVector<int> &order)
{
...
void ItemList::sortStateChanged(int state)
{
...
void ItemList::itemUniqueCheck(QJsonValue value, QString key, int index)
{
    bool isUnique = true;
...
void ItemList::addImageItem(int row, QJsonValue data)
{
    QImage image;
    QTableWidget *table = ui->arrayTable;
...
void ItemList::adoptItems(int col, int oldSize, int newSize)
{
    UNUSED(oldSize);
    QTableWidget *table = ui->arrayTable;
    if (col == 0) {
...
void ItemList::adoptText(int row)
{
    QString str;
...
void ItemList::openItemEditor(int index)
{
    if (index != -1) {
...
void ItemList::on_filterButton_toggled(bool checked)
{
    secOptions[FILTER_VISIBLE_KEY] = checked;
    ui->filterBox->setVisible(checked);
}

void ItemList::on_sortButton_toggled(bool checked)
{
    secOptions[SORT_VISIBLE_KEY] = checked;
    ui->sortBox->setVisible(checked);
}

void ItemList::on_backButton_clicked()
{
    goBack();
}

void ItemList::on_filterApplyButton_clicked()
{
    QFont font = ui->filterButton->font();
    filterStr = ui->filterEdit->text();
...
void ItemList::on_sortOrderButton_clicked()
{
    if (sortOrder)
        ui->sortOrderButton->setText(settings[AL_SORT_ORDER_DSC_KEY].toString());
    else
...
void ItemList::on_editButton_clicked()
{
...
void ItemList::on_removeButton_clicked()
{
    QString key;
    int index = -1;
    switch (section) {
...
void ItemList::on_disableFilter_clicked()
{
    ui->filterEdit->setText("");
    on_filterApplyButton_clicked();
}

void ItemList::on_disableSort_clicked()
{
    ui->sortEnable->setCheckState(Qt::Unchecked);
    sortStateChanged(0);
}
