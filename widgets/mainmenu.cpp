...
void MainMenu::initData(LogCollector *logDialog, QString fn, QJsonObject &data, QJsonObject &opt, bool changed)
{
    jsonPath = fn;
    allSettings = opt;
    jsonData = data;
...
bool MainMenu::checkFiles(QStringList &list)
{
...
bool MainMenu::checkAuthors(QStringList &list)
{
    bool isOk = true;
...
bool MainMenu::checkArts(QStringList &list)
{
    bool isOk = true;
...
QJsonArray MainMenu::mergeArrays(QString inPath, QString outPath, QString secSuffix,
...
void MainMenu::on_artsButton_clicked()
{
    showItemList(ArtsSection);
}

void MainMenu::on_authorsButton_clicked()
{
    showItemList(AuthorsSection);
}

void MainMenu::on_erasButton_clicked()
{
    showItemList(ErasSection);
}

void MainMenu::on_saveButton_clicked()
{
    if (isChanged) {
...
void MainMenu::on_goBackButton_clicked()
{
    if (isChanged) {
        QMessageBox msg(QMessageBox::Information, errorsMsg[ERRORS_TITLE_KEY].toString(),
...
void MainMenu::on_mergeButton_clicked()
{
    if (!isChanged) {
        merge();
    } else {
...
void MainMenu::on_checkInfo_linkActivated(const QString &link)
{
    UNUSED(link);
